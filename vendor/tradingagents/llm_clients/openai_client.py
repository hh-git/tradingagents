...
_HERMES_USER_AGENT = "HermesAgent/0.12.0"
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        if self.provider in _PROVIDER_CONFIG:
            ...
            if self.provider == "hermes" and not base_url:
                raise ValueError(
                    "Hermes provider requires backend_url/base_url for the "
        ...
        # The 86gamestore Hermes-compatible endpoint requires an explicit
        # User-Agent header on /v1/chat/completions requests.
        if self.provider == "hermes":
            default_headers = dict(self.kwargs.get("default_headers") or {})
            default_headers.setdefault("User-Agent", _HERMES_USER_AGENT)
